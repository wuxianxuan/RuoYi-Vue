...
public class StockAnalysisRecordController {
    private static final Logger log = LoggerFactory.getLogger(StockAnalysisRecordController.class);
    ...
    public AjaxResult downloadAnalysisData(StockAnalysisRequest request) {
        ...
        if (allDays.isEmpty()) {
            return AjaxResult.error("日期范围内无交易日数据");
        }
    ...
    private int processDate(String date) {
        ...
        try {
            ...
            ResponseEntity<StockResult> responseEntity = restTemplate.exchange(url, HttpMethod.GET, entity, StockResult.class);
        ...
        if (!records.isEmpty()) {
            stockAnalysisRecordService.batchSaveStockAnalysisRecords(records);
        }
        return records.size();
    }
}